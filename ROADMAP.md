...
| **Pi model-registry alignment** | v1.6.0 (in review) | The wizard/Judge/telemetry now read pi's own model registry (`ctx.modelRegistry.getAvailable()` / `find()` / `getProviderAuthStatus()` / `getApiKeyForProvider()`) instead of re-deriving a catalog from `models-store.json`. Fixes the picker showing a different, smaller list than `/model` (measured drift: 5 providers/413 models vs 39/1354; env-var and `models_json_command` credentials were invisible). Store paths remain as fallback. `src/model-source.ts`. |
